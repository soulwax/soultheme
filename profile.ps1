...
##### POSH THEME FILE ##############################################################################################################################
$poshThemeFile = "C:\Users\{0}\Documents\PowerShell\Themes\material.omp.json" -f $env:USERNAME
####################################################################################################################################################

##### USER ENVIRONMENT VARIABLES ###################################################################################################################
$WorkFoldersPath = "$env:USERPROFILE\Work Folders"
$AlternateDevPath = "C:\Users\{0}\Documents\Workspace" -f $env:USERNAME
$DevPath = "D:\Development"
$DevPathGer = "D:\Entwicklung"
$GamesPath = "D:\Games"
...
# Creates a OneDrive shortcut, if it exists in registry
...
if (Test-Path $DevPath) {
    New-PSDrive -Name Dev -PSProvider FileSystem -Root $DevPath -Description "Development"
    function Dev: { Set-Location Dev: }
} elseif (Test-Path $DevPathGer) {
    New-PSDrive -Name Dev -PSProvider FileSystem -Root $DevPathGer -Description "Development"
    function Dev: { Set-Location Dev: }
} 
elseif (Test-Path $AlternateDevPath) {
    New-PSDrive -Name Dev -PSProvider FileSystem -Root $AlternateDevPath -Description "Development"
    function Dev: { Set-Location Dev: }
}
